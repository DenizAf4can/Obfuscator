...
function bit.lshift(x, n)
    return (x * 2^n) % 4294967296 -- Left shift
end
function bit.rshift(x, n)
    return math.floor(x % 4294967296 / 2^n) -- Right shift
end

-- _W Fonksiyonu (Lua 5.2+ uyumlu)
local function _W(f)
    local env = setmetatable({}, { __index = _ENV }) -- Lua 5.2+ için _ENV kullanıldı
    return f(env) or env
end

-- Bit API'sini tanımla
local bit_api = _W(function(_ENV)
